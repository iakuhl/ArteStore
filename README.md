...
| Arquivo | Descrição |
|--------|------------|
| `estruturas.h` / `defines.h` | Definição de constantes, tipos (Data, Telefone, redeSocial, Artista, Obra, Colaboracao) e listas dinâmicas (ListaArtistas, ListaObras, ListaColaboracoes). |
| `utils.h` / `utils.c` | Funções utilitárias de entrada: `lerInteiro`, `lerString`, `limparBuffer`, `validarCPF`, `escolherOpcao`. |
