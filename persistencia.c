...
int carregarArtistas(ListaArtistas *lista)
{
	...
	for (i = 0; i < total; i++)
	{
		...
		if (totalRedes > 0)
		{
			...
			{
				if(fread(a.redesSociais[j].plataforma, sizeof(char), TAM_TEXTO_PEQUENO, arquivo) != TAM_TEXTO_PEQUENO)
				{
					free(a.redesSociais);
					free(a.telefones);
					fclose(arquivo);
				...
				a.redesSociais[j].plataforma[TAM_TEXTO_PEQUENO - 1] = '\0';
...
bool salvarArtistas(const ListaArtistas *lista)
{
    ...
    for (i = 0; i < lista->total; i++)
    {
        ...
        {
            if(fwrite(a->redesSociais[j].plataforma, sizeof(char), TAM_TEXTO_PEQUENO, arquivo) != TAM_TEXTO_PEQUENO)
            {
                fclose(arquivo);
                return false;
            }
